...
public class LandingController implements Initializable {
    ...
    @FXML
    private TextField payment_student_studentid;
    ...
    public void paymentsSelect() {
        ...
        payment_student_studentid.setText(String.valueOf(paymentD.getStudent_id())); // Convert int to String
    }


    public int getPaymentID(int student_id, int payment_amount, Date payment_date) throws SQLException {
        int paymentId = -1;
        String sql = "SELECT payment_id FROM payments WHERE student_id = ? AND payment_amount = ? AND payment_date = ?";

        try (Connection connection = DatabaseConnection.getConnection();
             PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setInt(1, student_id);
            pstmt.setInt(2, payment_amount);
    ...
    public void paymentDelete() throws SQLException {
        ...
        int studentId = Integer.parseInt(payment_student_studentid.getText());
        ...
        int paymentId = getPaymentID(studentId, paymentAmount, sqlDate);
    ...
    public String[] getStudentNameById(int studentId) {
        String[] studentName = new String[2]; // Array to store first name and last name
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                studentName[0] = rs.getString("first_name");
                studentName[1] = rs.getString("last_name");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void paymentAdd() throws SQLException {
        ...
        if (studentName[0] != null && studentName[1] != null) {
            connection = DatabaseConnection.getConnection();
            try (PreparedStatement insertStatement = connection.prepareStatement(insertData)) {
                insertStatement.setInt(1, studentId);
                insertStatement.setString(2, studentName[0]); // First name
                insertStatement.setString(3, studentName[1]); // Last name
            ...
            showAlert("Student Not Found", "No student found with this ID");
        }
    }
